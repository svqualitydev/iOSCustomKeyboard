...
extension KeyboardViewController {
    ...
    func topButton(for action: KeyboardAction, distribution: UIStackView.Distribution = .equalSpacing) -> UIView {
        let view = TopButton.fromNib(owner: self)
        view.setup(with: action, in: self, distribution: .fillEqually)
        return view
    }
    
    func topButtonRow(for actions: KeyboardActionRow, distribution: UIStackView.Distribution) -> KeyboardStackViewComponent {
        KeyboardButtonRow(actions: actions, distribution: distribution) {
            topButton(for: $0, distribution: distribution)
        }
    }
}
