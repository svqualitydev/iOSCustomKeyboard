...
extension DemoKeyboard {
    ...
    static func topActions(for vc: KeyboardViewController) -> KeyboardActionRow {
        let actions = [KeyboardAction.custom(name: CustomButtonName.payments.rawValue),
                       KeyboardAction.custom(name: CustomButtonName.templates.rawValue),
                       KeyboardAction.custom(name: CustomButtonName.scheduling.rawValue)]
        return actions
    }
