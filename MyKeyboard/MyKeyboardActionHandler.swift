...
class MyKeyboardActionHandler: StandardKeyboardActionHandler {
    ...
    override func tapAction(for action: KeyboardAction, view: UIView) -> GestureAction? {
        ...
        case .custom(let name):
            if name == CustomButtonName.back.rawValue {
                return switchToLastKeyboard
            } else {
                return switchToTokenKeyboard
            }
...
@objc extension MyKeyboardActionHandler {
...
private extension MyKeyboardActionHandler {
    ...
    func switchToTokenKeyboard() {
        demoViewController?.keyboardType = .custom(KeyboardCustomType.token.rawValue)
    }
    
    func switchToLastKeyboard() {
        switchToAlphabeticKeyboard(.lowercased)
    }
}
